public abstract class ValidatorBST<E extends Comparable<E>> implements IBTValidator<E> {
    @Override
    public boolean validAdd(IBinTree<E> oldTree, E elt, IBinTree<E> newTree) {
        ...
        boolean bstInv = bstInvariant(newTree);
    ...
    public boolean bstInvariant(IBinTree<E> someTree){
        if(someTree.isEmpty()) { return true; }
        else {
            E root = someTree.getRoot();;
            IBinTree<E> left = someTree.getLeft();
            ...
            return biggerThanAll(root, left) &&
